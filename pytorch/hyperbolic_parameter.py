...
class KleinParameter(RParameter):
    ...
    def modify_grad_inplace(self):
        ...
        hyper_b   = (1 - w_norm**2).double()
        # hyper_b   = (1 - w_norm**2)
        # self.grad   *= hyper_b

        data_points = self.data.shape[0]
        identity = torch.eye(data_points).double()
        test = torch.mm(self.data, torch.transpose(self.data,0,1)).double()
        entire = identity / hyper_b + test / torch.mul(hyper_b, hyper_b)
        inv = torch.inverse(entire)

        self.grad = torch.mm(inv, self.grad)
        self.grad.clamp_(min=-10000.0, max=10000.0)

    # def dist_h(p,q):
        # if torch.eq(p,q): return 0

        # va = torch.norm(p - q)**2
        # vb = 2 * KleinParameter.dot_h(torch.transpose(p,0,1) - torch.transpose(q,0,1), q)
        # vc = KleinParameter.dot_h(q, q)

        # sol_1 = (-vb + torch.sqrt(vb**2 - 4*va*vc)) / (2*va)     # big
        # sol_2 = (-vb - torch.sqrt(vb**2 - 4*va*vc)) / (2*va)     # small
    ...
    def dist_h(p,q):
        ...
        a = aval*(p-q) + q
        b = bval*(p-q) + q
        return 0.5 * torch.log(torch.norm(torch.abs(p-b)) * torch.norm(torch.abs(q-a)) / (torch.norm(torch.abs(p-a)) * torch.norm(torch.abs(q-b))) )

        # when calculating actual dist, make sure denom is not too small
